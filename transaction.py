...
def transfer():
    ...
    if receiver_id == auth.current_user:
    ...
    if cur_balance - send_amount < minimum_balance:
        ...
        return
    try:
        cur.execute("UPDATE accounts SET balance= balance - %s WHERE account_id=%s",(send_amount,auth.current_user))
        ...
        cur.execute("""INSERT INTO transactions (account_id, transaction_type, amount, balance_after, description) VALUES (%s, %s, %s, %s, %s)""",(auth.current_user, "transfer", send_amount, sender_new_balance, f"Transfer to account {receiver_id}"))
        cur.execute("""INSERT INTO transactions (account_id, transaction_type, amount, balance_after, description) VALUES (%s, %s, %s, %s, %s)""",
